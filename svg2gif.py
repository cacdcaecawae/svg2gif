...
animation_timers = [_clean_time_element(time_element.get("dur"))
		 for time_element in soup.find_all('animate')]
...
if total_time_animated < 20:
	...
	temp_filename = f"TMP_{os.path.basename(FILE_NAME)}"
...
if USE_TMP_PATH:
	temp_filename = f"TMP_{os.path.basename(FILE_NAME)}"
	driver.get(f"file:///{ABSOLUTE_FILE_PATH}/{temp_filename}")
else:
...
# 输出到当前目录，只保留文件名
fp_out = f'{os.path.basename(FILE_NAME).replace(".svg",".gif")}'
...
# Optional delete of chrome logs (may not exist)
